<x-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <!-- RIGHT COLUMN: Sidebar (Details & Controls) -->
            <div class="lg:col-span-1 space-y-6">
                
                <!-- Ticket Details (Visible to everyone) -->
                <div class="bg-zinc-200 dark:bg-gray-800 shadow rounded-lg overflow-hidden">
                    <div class="px-4 py-5 sm:px-6 border-b border-gray-200 dark:border-gray-700">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">
                            Ticket Details
                ...
                <!-- Employee Controls (Visible ONLY to Staff) -->
                @if(auth()->check() && (auth()->user()->role === \App\Enums\UserRole::E_EMPLOYEE || auth()->user()->role === \App\Enums\UserRole::E_ADMIN))
                    <div class="bg-zinc-200 dark:bg-gray-800 shadow rounded-lg p-6 sticky top-24">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white border-b border-gray-300 dark:border-gray-700 pb-3 mb-4">
                            Ticket Management
                        </h3>

                        <form action="{{ route('tickets.assign', $ticket) }}" method="POST" class="mb-6">
                            @csrf
                            @method('PATCH')
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Agent Assignment</label>
                            
                            <div class="flex flex-col space-y-2">
                                <!-- The Button or Badge -->
                                @if($ticket->employee_profile_id !== auth()->id())
                                    <button type="submit" class="w-full text-sm px-3 py-2 bg-indigo-100 text-indigo-700 hover:bg-indigo-200 dark:bg-indigo-900/50 dark:text-indigo-300 dark:hover:bg-indigo-800 rounded-md transition-colors font-medium">
                                        Assign to me
                                    </button>
                                @else
                                    <span class="w-full text-center text-sm px-3 py-2 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 rounded-md font-medium">
                                        Assigned to you
                                    </span>
                                @endif

                                <!-- The Assignee Name (appears below if an employee is assigned) -->
                                @if($ticket->assignedEmployee)
                                    <div class="text-xs text-center text-gray-500 dark:text-gray-400 mt-1">
                                        Currently assigned to: 
                                        <span class="font-medium text-gray-700 dark:text-gray-300">
                                            {{ $ticket->assignedEmployee->display_name ?? $ticket->assignedEmployee->name }}
                                        </span>
                                    </div>
                                @else
                                    <div class="text-xs text-center text-gray-400 dark:text-gray-500 italic mt-1">
                        ...
                        <form action="{{ route('tickets.update', $ticket) }}" method="POST" class="space-y-4">
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            ...
                            <div>
                                ...
                                <select id="category" name="category" class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    @foreach($categories as $category)
                                        <option value="{{ $category->value }}" @selected($ticket->category->value === $category->value)>
